...
`config.json` — edit the `family` array to add or remove family members, then restart the server. The list is served to the frontend via `GET /api/config` and validated server-side on every write.

## API

| Method | Path | Body / Query | Notes |
|--------|------|-------------|-------|
| GET | `/api/config` | — | Returns `{ family: [...] }` |
